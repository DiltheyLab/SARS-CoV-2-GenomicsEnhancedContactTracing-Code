...
setwd("C:/Users/joh/Desktop/HomeOffice/NUM/code and data for paper")
...
g <- read_graph("graph_final_undirected.gml",format=c("gml"))
edgesNew <- as_long_data_frame(g)
...
##Load general data
load("data for outbreak analysis")
dm <- read.delim("C:/Users/joh/Desktop/HomeOffice/NUM/code and data for paper/distancematrix_prefiltered.tab")
case_matching <- read.csv("C:/Users/joh/Desktop/HomeOffice/NUM/Moritz_ausbruch/case_matching.csv", header=FALSE)
names(case_matching) <- c("caseID", "SeqID")

## Select nodes of the final graph
nodes <- nodes[nodes$CaseID %in% graphIDs,]

edgesGraph <- data.frame(CaseID_Index= edgesNew$from_label,
                         CaseID_Kontaktperson= edgesNew$to_label,
                         Typ= edgesNew$type)

nodes$lost <- 1
nodes$lost[nodes$CaseID %in% graphIDs] <- 0

search <- nodes[nodes$lost == 1,]
search$CaseID
...
## Replace nodes seq info
nodes$SeqID <- case_matching$SeqID[match(nodes$CaseID, case_matching$caseID)]
nodes2$SeqID <- case_matching$SeqID[match(nodes2$CaseID, case_matching$caseID)]
...
### Modify graph table for easiy lookup
...
# 
# save(getGeneticDistance, getCaseID, nodes2, nodes, edgesGraph, dm, edge2, edge,
#      file="data")

#save.image("C:/Users/joh/Desktop/HomeOffice/NUM/code and data for paper/data.RData")
...
newOutbreaks <- read_excel("C:/Users/joh/Desktop/HomeOffice/NUM/Moritz_ausbruch/20240111 Abfrage SurvNet Ausbrüche NEW.xlsx")
...
outbreakTableNamePath <- "C:/Users/joh/Desktop/HomeOffice/NUM/Moritz_ausbruch/Outbreak summary table.xlsx"

## Load outbreak table of medical student
eva <- read_excel(outbreakTableNamePath )

outbreak$type3[outbreak$type2 == "HEIM"] <- eva$Category[match(outbreak$AusbruchInfo_NameGA[outbreak$type2 == "HEIM"] , eva$`Outbreak ID`)]
outbreak$type3[outbreak$AusbruchInfo_NameGA == "CVD-DÜS-HEIM-2021-0031"] <-  "residential establishment"

outbreak$type3[outbreak$type3 == "residential establishment"] <- "Residential Care Facility"
outbreak$type3[outbreak$type3 == "asylum accommodation"] <- "Refugee Accommodation"
outbreak$type3[outbreak$type3 == "Kindergarten"] <- "Kindergarten/daycare"

outbreak_save <- outbreak

##### Table creation

## Generate outbreak suppl table
outbreakTableNamePath <- "C:/Users/joh/Desktop/HomeOffice/NUM/code and data for paper/Moritz-Table Final.xlsx"
...
outbreak$AusbruchInfo_Aktenzeichen <- outbreaks$AusbruchInfo_Aktenzeichen[match(outbreak$AusbruchInfo_NameGA, outbreaks$AusbruchInfo_NameGA)]
   ...
   if(nrow(cur_outbreack) != 0){
      ...
      if(exists("connected")){
         ...
         for(everyCase in  seq_len(nrow(cases))){
            ...
            nodes2$dateDiff <- 0
            nodes2$dateDiff <- as.numeric(lapply(nodes2$MeldedatumDate, function(x){
...
for(i in 1:nrow(os)){
   ...
   eGlI <- subOut$externalGenLinksInverstigated
...
write.csv2(os, file="os.csv", row.names=F)
...
write.csv2(osOutbreak, file="os_Outbreak.csv", row.names=F)
...
write.csv2(ol, file="outbreak links")
...
write.csv2(ol, file="ol.csv", row.names=F)


save.image("C:/Users/joh/Desktop/HomeOffice/NUM/code and data for paper/FinalData2.RData")
...
save.image("C:/Users/joh/Desktop/HomeOffice/NUM/code and data for paper/sofarSoGood.RData")

#load("sofarSoGood.RData")
...
save.image("C:/Users/joh/Desktop/HomeOffice/NUM/coco random pair new data Angestecktbei.RData")
...
DFpairs <- DFpairs[, names(DFpairs) %in% saveColumns]
anbeiGen <- anbeiGen[, names(anbeiGen) %in% saveColumns]

## save
save(DFpairs, anbeiGen, file="Matched pairs allContact")
...
library(readxl)
anbei_with <- read_excel("C:/Users/joh/Desktop/HomeOffice/NUM/data send by coco/AngestecktBei pairs (1).xlsx", 
                         sheet = "pairs with genetic info")
anbei_with$type <- "with distance"
anbei_without <- read_excel("C:/Users/joh/Desktop/HomeOffice/NUM/data send by coco/AngestecktBei pairs (1).xlsx", 
...
library(readxl)
contacts <- read_excel("C:/Users/joh/Desktop/HomeOffice/NUM/data send by coco/contact person mit Gen. Info.xlsx", 
...
## add new context
KPcontexts <- read.csv("C:/Users/joh/Desktop/HomeOffice/NUM/code and data for paper/KP Kontaktumfeld 20230321.csv", sep=";")
...
gencon2 <- data.frame(Infektionskontext_engl= gencon$Infektionskontext_engl, genetic_validation= gencon$genetic_validation)
chisq.test(gencon2$Infektionskontext_engl, gencon2$genetic_validation)
p <- 0.9737  ## => context not significantly different
...
data <- matrix(
   c(1527, 299, 1015, 367), # Flattened data: Rejected and Accepted for A1, then for A2
...
data <- matrix(
   c(162, 79 , 61, 37), # Flattened data: Rejected and Accepted for A1, then for A2
   nrow = 2,          # Number of rows (conditions)
   byrow = TRUE       # Fill matrix row-wise
)
...
nodes_subsampling <- read.csv("C:/Users/joh/Desktop/HomeOffice/NUM/code and data for paper/Subsampling Dataset.csv", sep=";")
...
save(documentCaseIDsUpsampling, file="CaseIDs upon upsampling")
...
inSource <- read.delim("C:/Users/joh/Desktop/HomeOffice/NUM/code and data for paper/infsources_per_sequenced_cases_connected_components_0.tsv")
...
KPcontexts <-  read.delim("C:/Users/joh/Desktop/HomeOffice/NUM/code and data for paper/KP and context.csv", comment.char="#")
...
infectedByWithGeneticInfo <- read_excel("C:/Users/joh/Desktop/HomeOffice/NUM/data send by coco/AngestecktBei pairs (1).xlsx", 
                                        sheet = "pairs with genetic info")

infectedByNoGeneticInfo <- read_excel("C:/Users/joh/Desktop/HomeOffice/NUM/data send by coco/AngestecktBei pairs (1).xlsx", 
                                      sheet = "pairs without genetic info")
...
g <- ggplot(test, aes(Var1, Freq ))+ geom_bar(stat="identity",position = "identity")+
   geom_text(aes(label = percent), vjust = -0.2, size=3)+
   xlab("Time between case registration dates")+ylab("Number of contact person pairs")+theme(legend.title=element_blank())+
   scale_fill_brewer(palette="Set2")+
   theme(text = element_text(size=15))
...
g <- ggplot(test, aes(Var1, Freq ))+ geom_bar(stat="identity",position = "identity")+
   geom_text(aes(label = percent), vjust = -0.2, size=3)+
   xlab("Time between case symptom start dates")+ylab("Number of contact person pairs")+theme(legend.title=element_blank())+
   scale_fill_brewer(palette="Set2")+
   theme(text = element_text(size=15))
...
curGenContact$classification2[curGenContact$indexSearch %in% Rest$indexSearch] <- "Contact tracing link"
curGenContact$classification2[!curGenContact$indexSearch %in% Rest$indexSearch] <- "No contact tracing link"
...
curGenContact$classification[curGenContact$indexSearch %in% RestSameName$indexSearch] <- "Same Name"
curGenContact$classification[!curGenContact$indexSearch %in% RestSameName$indexSearch] <- "Not same name"
